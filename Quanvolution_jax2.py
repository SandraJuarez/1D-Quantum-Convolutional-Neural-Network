...
class QuanvLayer1D(nn.Module):
    ...
    def Qcircuit(self, inputs,weights):
        ...
        for b in range(bs):
            for k in range(0, l_out, self.stride):
                ...
                inp=jnp.array(inputs[b, 0, k:k + self.kernel_size])
                q_results = circuit(inp,weights)
    ...
    def __call__(self, input):
        ...
        out = nn.Dense(features=1)(out)    #features=1 because we are predicting only 1 point
        return out