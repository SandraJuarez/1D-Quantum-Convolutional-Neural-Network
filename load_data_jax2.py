...
def data(dataset):
    ...
    if dataset=='sp500_md': #este dataset es multidimensional
        #cargamos el csv
        data = np.genfromtxt('datasets/sp500.csv', delimiter=',',skip_header=1)
        ...
        data = np.genfromtxt('data/BTC.csv', delimiter=',', skip_header=1)
        data=data[:,1:-1]
        #data=np.transpose(data)
        ...
        data = np.genfromtxt('datasets/BTC.csv', delimiter=',', skip_header=1)
        ...
        data = np.genfromtxt('datasets/USD-EURO-Time-Series.csv', delimiter=',')
        ...
        data = mackey_glass(sample_len=3024,seed=0)[0]
        scaled_data_mackey_glass = (data-np.min(data))/(np.max(data)-np.min(data)) #scaler.fit_transform(data)
        scaled_data = np.zeros((3000,5))
        for t in range(18,3018):
 ...
 #NEW DATASETS HERE! 
 # hello, a partir de aqui meti los datasets nuevos, fish data, soil, US inflation y oil price. 
 # fish data
    elif dataset=='fish_data': 
        data = np.genfromtxt('datasets/fish_data.csv', delimiter=',',skip_header=1)
        data=data[:,1:-1]
        data=np.transpose(data)
        scaled_data_fish_data=(data-np.min(data))/(np.max(data)-np.min(data))
        features=int(data.shape[0])
        scaled_data = np.zeros((3760,4,6))
        for t in range(4,3764):
            idx = t-4 # row index. 
            scaled_data[idx,:,:] =np.transpose([scaled_data_fish_data[t-4],scaled_data_fish_data[t-3],scaled_data_fish_data[t-2],
                                    scaled_data_fish_data[t-1],scaled_data_fish_data[t],scaled_data_fish_data[t+1]])
        scaled_data_train = scaled_data[0:3000,:,0:6]
        scaled_data_test = scaled_data[3000:,:,0:6]
        X_train = scaled_data_train[:,:,:5]  
        Y_train = scaled_data_train[:,:,5]
...
#crude oil price
    elif dataset=='oil_price': 
        data = np.genfromtxt('datasets/crude-oil-price.csv', delimiter=',',skip_header=1)
        data=data[:,1:-1]
        data=np.transpose(data)
        scaled_data_oil_price=(data-np.min(data))/(np.max(data)-np.min(data))
        features=int(data.shape[0])
        scaled_data = np.zeros((490,4,6))
        for t in range(4,490):
            idx = t-4 # row index. 
            scaled_data[idx,:,:] =np.transpose([scaled_data_oil_price[t-4],scaled_data_oil_price[t-3],scaled_data_oil_price[t-2],
                                    scaled_data_oil_price[t-1],scaled_data_oil_price[t],scaled_data_oil_price[t+1]])
        scaled_data_train = scaled_data[0:490,:,0:6]
        scaled_data_test = scaled_data[490:,:,0:6]
        X_train = scaled_data_train[:,:,:5]  
        Y_train = scaled_data_train[:,:,5]
        X_test = scaled_data_test[:,:,:5]  
        Y_test = scaled_data_test[:,:,5]
...
#original soil dataset
    elif dataset=='soil_dataset': 
        data = np.genfromtxt('datasets/OriginalSoilDataset.csv', delimiter=',',skip_header=1)
        data=data[:,1:-1]
        data=np.transpose(data)
        scaled_data_soil_dataset=(data-np.min(data))/(np.max(data)-np.min(data))
        features=int(data.shape[0])
        scaled_data = np.zeros((3760,4,6))
        for t in range(4,3764):
            idx = t-4 # row index. 
            scaled_data[idx,:,:] =np.transpose([scaled_data_soil_dataset[t-4],scaled_data_soil_dataset[t-3],scaled_data_soil_dataset[t-2],
                                    scaled_data_soil_dataset[t-1],scaled_data_soil_dataset[t],scaled_data_soil_dataset[t+1]])
        scaled_data_train = scaled_data[0:3000,:,0:6]
        scaled_data_test = scaled_data[3000:,:,0:6]
        X_train = scaled_data_train[:,:,:5]  
        Y_train = scaled_data_train[:,:,5]
...
#us inflation
    elif dataset=='us_inflation': 
        data = np.genfromtxt('datasets/US_inflation_rates.csv', delimiter=',',skip_header=1)
        data=data[:,1:-1]
        data=np.transpose(data)
        scaled_data_us_inflation=(data-np.min(data))/(np.max(data)-np.min(data))
        features=int(data.shape[0])
        scaled_data = np.zeros((900,4,6))
        for t in range(4,900):
            idx = t-4 # row index. 
            scaled_data[idx,:,:] =np.transpose([scaled_data_us_inflation[t-4],scaled_data_us_inflation[t-3],scaled_data_us_inflation[t-2],
                                    scaled_data_us_inflation[t-1],scaled_data_us_inflation[t],scaled_data_us_inflation[t+1]])
        scaled_data_train = scaled_data[0:900,:,0:6]
        scaled_data_test = scaled_data[900:,:,0:6]
        X_train = scaled_data_train[:,:,:5]  
        Y_train = scaled_data_train[:,:,5]
        X_test = scaled_data_test[:,:,:5]  
        Y_test = scaled_data_test[:,:,5]
