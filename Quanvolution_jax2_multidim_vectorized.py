...
class QuanvLayer1D(nn.Module):
    ...
    def Qcircuit(self, inputs,weights):
        ...
        def process_kernel_segment(segment, w):
            # Apply the quantum circuit on the segment with weights
            return jnp.asarray(circuit(segment, w))
        # Function to extract segments for each batch
        def extract_segments(batch_input):
            segments = []
            for k in range(0, l_out * self.stride, self.stride):
                segment = jax.lax.slice(batch_input, (0, k), (ch, k + self.kernel_size))
                segments.append(segment)
            segments = jnp.stack(segments, axis=0)
            return segments
        # Vectorized segment extraction and circuit application
        def apply_circuit(batch_input):
            segments = extract_segments(batch_input)
            vmap_results = jax.vmap(process_kernel_segment, in_axes=(0, None))(segments, weights)
            return vmap_results

        # Apply the vectorized process to each batch
